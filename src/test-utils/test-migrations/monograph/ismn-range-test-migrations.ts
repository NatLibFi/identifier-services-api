...
export async function createIsmnRangeTable(db: Kysely<Database>): Promise<void> {
  await db.schema
    .createTable('ismn_range')
    .addColumn('id', 'integer', (col) => col.primaryKey().notNull().autoIncrement())
    .addColumn('gs1', 'varchar(3)', (col) => col.notNull())
    .addColumn('registration_group', 'varchar(1)', (col) => col.notNull())
...
export async function dropIsmnRangeTable(db: Kysely<Database>): Promise<void> {
  await db.schema.dropTable('ismn_range').execute();
}
