...
export async function createMonographPublisherRequestTable(db: Kysely<Database>): Promise<void> {
  await db.schema
    .createTable('monograph_publisher_request')
    ...
    .addColumn('modified_by', 'varchar(30)', (col) => col.notNull())
    .execute();
}

export async function dropMonographPublisherRequestTable(db: Kysely<Database>): Promise<void> {
  await db.schema.dropTable('monograph_publisher_request').execute();
}
