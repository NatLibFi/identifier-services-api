  ...
  sequelize.define(
    'publisherIsbn', {
      ...
      previousNames: {
        ...
        // Previous data has been saved as stringified JSON with property
        // "name" having an array of values.
        get() {
          const value = this.getDataValue('previousNames');
          try {
            const parsedValue = JSON.parse(value).name;
            return parsedValue;
          } catch {
            return [];
          }
      ...
      classification: {
        ...
        type: DataTypes.STRING(50),
        get() {
          const value = this.getDataValue('classification');
          if (value === '') {
            return [];
          }
    ...
    {
      tableName: `${TABLE_PREFIX}_isbn_registry_publisher`,
      underscored: true,
      createdAt: 'created',
      updatedAt: 'modified',
      indexes
