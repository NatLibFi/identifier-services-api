...
export const createIsmnRangeSchema = z
  .object({
    gs1: z.enum(Object.values(ISMN_VALID_GS1)),
    registration_group: z.enum(Object.values(ISMN_VALID_REGISTRATION_GROUPS)),
    range_begin: z.string().min(3).max(7).regex(numbersOnlyString),
    range_end: z.string().min(3).max(7).regex(numbersOnlyString),
  })
  .strict()
...
export const updateIsmnRangeSchema = z
  .object({
    active: z.boolean().optional(),
    range_begin: z.string().min(3).max(7).regex(numbersOnlyString).optional(),
    range_end: z.string().min(3).max(7).regex(numbersOnlyString).optional(),
  })
  .strict()
  .superRefine((data, ctx) => {
...
export type CreateIsmnRangeHttp = z.infer<typeof createIsmnRangeSchema>;
export type UpdateIsmnRangeHttp = z.infer<typeof updateIsmnRangeSchema>;
