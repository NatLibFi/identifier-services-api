...
export const validateCreateMessageType = {
  name: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(50).required(),
  description: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(200).required()
};

export const validateUpdateMessageType = {
  name: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(50),
  description: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).allow('').max(200)
};

export const validateCreateMessageTemplate = {
  messageTypeId: Joi.number().integer().required(),
  name: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(50).required(),
  subject: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(150).required(),
  langCode: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).regex(regexPatterns.langCode).required(),
  message: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(65000).required()
};

export const validateUpdateMessageTemplate = {
  messageTypeId: Joi.number().integer(),
  name: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(50),
  subject: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(150),
  langCode: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).regex(regexPatterns.langCode),
  message: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).max(65000)
};
...
export const validateLoadTemplateIsbn = {
  ...
  code: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).valid(...Object.keys(MESSAGE_TYPE_CONFIG_ISBN)).required(),
...
export const validateLoadTemplateIssn = {
  // Code must be one mapped to a desired message type in env config
  code: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).valid(...Object.keys(MESSAGE_TYPE_CONFIG_ISSN)).required(),
  publisherId: Joi.number().integer().required(),
  formId: Joi.number().integer()
};
...
export const validateSendMessageIsbn = {
  ...
  langCode: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).regex(regexPatterns.langCode).required(),
  recipient: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).regex(regexPatterns.email).required(),
  subject: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).required(),
  messageBody: Joi.string().regex(regexPatterns.utf8mb4, {invert: true}).required()
};
