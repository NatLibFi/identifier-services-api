...
export default function (permissionMiddleware) {
  ...
  return new Router()
    ...
    .put('/:id', permissionMiddleware('publisherRequests', 'update'), celebrate({
      [Segments.PARAMS]: validateRequestId,
      [Segments.BODY]: validateUpdatePublisherRequestIsbn
    }), update)
    .delete('/:id', permissionMiddleware('publisherRequests', 'delete'), celebrate({
      [Segments.PARAMS]: validateRequestId
    }), remove)
  ...
  async function create(req, res, next) {
    try {
      const result = await publisherRequests.create(req.body, req.user);
      if (result) {
  ...
  async function read(req, res, next) {
    try {
      const result = await publisherRequests.read(req.params.id);
  ...
  async function update(req, res, next) {
    try {
      const result = await publisherRequests.update(req.params.id, req.body, req.user);
  ...
  async function remove(req, res, next) {
    try {
      const result = await publisherRequests.remove(req.params.id);
  ...
  async function query(req, res, next) {
    try {
      const result = await publisherRequests.query(req.body);
